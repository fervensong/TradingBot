...
const gekkoManager = require('../../web/state/gekkoManager');
...
Base.prototype.sendError= function(errMsg){
  console.log(errMsg);
  const gekkoManager1 = cache.get('gekkos');
  console.log(gekkoManager1);
  //this.end();
  //return done();
...
Base.prototype.advice = function(newDirection) {
  ...
  let trigger;
  console.log("base trading advice: ");
  ...
  if(_.isObject(newDirection)) {
    ...
    if(newDirection.direction === this._currentDirection) {
      console.log("cur direction is already running.");
      return;
    }
  ...
  //console.log(this.propogatedAdvices);
  return this.propogatedAdvices;
}

Base.prototype.alert = function(newDirection) {
 if(!newDirection) return 0;

 console.log("new Direction :"+ newDirection);

  this.emit('alert', {alertData: {direction:newDirection}});
  const advice = {
    id: 'advice-' + this.propogatedAdvices,
    recommendation: newDirection
  };
  this.emit('advise',advice);
